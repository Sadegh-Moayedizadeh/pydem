...
from pydem.geometry.two_dimensional_operations import is_crossed, is_perpendecular, distance
...
class Polygon(object):
    ...
    def __init__(self, *vertices: Point) -> None:
        ...
        if self.number_of_vertices < 3:
            raise BadEntries('the number of given points should be equal or more than three')
        dd = defaultdict(int)
        ...
        for edge1 in self.edges:
            for edge2 in self.edges:
                if edge1 != edge2 and is_crossed(edge1, edge2):
                    raise BadEntries('the given vertices form a polygon with intersecting edges')
        
    @classmethod
    def as_regular(cls, centre: Point, diameter: float, number_of_vertices: int) -> 'Polygon':
        ...
        points = [circumcircle.get_point_on_perimeter(angle) for angle in count(0, 2*np.pi, 2*np.pi/n)]
        return cls(*points)
    ...
    def __init__(self, *vertices: Point) -> None:
        ...
        for i in range(n):
            if not is_perpendecular(edges[i], edges[i-1]):
                raise BadEntries('the given vertices do not form a rectangle with perpendecular edges')
...
class Circle(object):
    ...
    def __eq__(self, other: Any) -> bool:
        ...
        if isinstance(other, Circle) and self.centre == other.centre and self.diameter == other.diameter:
            return True
        return False
...
class Line(object):
    ...
    def get_x(self, y: float) -> float:
        ...
        try:
            ...
            raise BadEntries('the line instance has the slope of zero, asking for an \'x\' coordinate is invalid in this case')
        return res
    ...
    def navigator(self, start: Point, end: Point, step: float) -> Point:
        ...
        if not is_on_perimeter(start, self) or not is_on_perimeter(end, self):
            raise BadEntries('the given start and end points should be loacated on the Line instance')
        dist = 0
    ...
    def __eq__(self, other: Any) -> bool:
        ...
        if isinstance(other, Line) and self.slope == other.slope and self.width == other.width:
            return True
        return False
...
class LineSegment(object):
    ...
    def __init__(self, end1: Point, end2: Point) -> None:
        ...
        if end1 == end2:
            raise BadEntries('the given points have the same coordinates, they can\'t produce a line segment')
        self.end1 = end1
        self.end2 = end2
    
    @classmethod
    def from_point_and_inclination(cls, point: Point, inclination: float, size: float) -> 'LineSegment':
    ...
    def get_x(self, y: float) -> float:
        ...
        if not is_on_perimeter(point, self):
            raise BadEntries('the current LineSegment instance does not reach the given width')
        return x
    ...
    def get_y(self, x: float) -> float:
        ...
        point = Point(x, y)
        if not is_on_perimeter(point, self):
            raise BadEntries('the current LineSegment instance does not reach the given length')
        return y
    ...
    def __eq__(self, other: Any) -> bool:
        ...
        if isinstance(other, LineSegment) and self.end1 == other.end1 and self.end2 == other.end2:
            return True
        return False